...
COORDS="$SCRIPT_DIR/coords.json"

# Read dimensions from coords.json (generated by cleanup.py)
if [ ! -f "$COORDS" ]; then
    echo "Error: coords.json not found at $COORDS"
    echo "Run cleanup.py first to generate frame.png and coords.json."
    exit 1
fi

FRAME_W=$(python3 -c "import json; print(json.load(open('$COORDS'))['frame_width'])")
FRAME_H=$(python3 -c "import json; print(json.load(open('$COORDS'))['frame_height'])")
SCREEN_X=$(python3 -c "import json; print(json.load(open('$COORDS'))['screen_left'])")
SCREEN_Y=$(python3 -c "import json; print(json.load(open('$COORDS'))['screen_top'])")
SCREEN_W=$(python3 -c "import json; print(json.load(open('$COORDS'))['screen_width'])")
SCREEN_H=$(python3 -c "import json; print(json.load(open('$COORDS'))['screen_height'])")
